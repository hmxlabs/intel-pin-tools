...
KNOB< string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "insmixamd.out", "specify file name for insmixamd profile ");
...
KNOB< BOOL > KnobOnlyDivergentAsm(KNOB_MODE_WRITEONCE, "pintool", "divergent", "1", "Ignore all asm with identical behaviour on AMD and Intel");
...
static const UINT32 divergentAsm[] = {
    XED_ICLASS_RSQRTPS,
    XED_ICLASS_RSQRTSS,
    XED_ICLASS_RCPPS,
    XED_ICLASS_RCPSS,
...
const static UINT32 numDivergentAsm = sizeof(divergentAsm) / sizeof(UINT32);


static BOOL IsDivergentOp(UINT32 opcode)
{
    for(UINT32 i=0; i < numDivergentAsm; i++) {
        if( opcode == divergentAsm[i] )
            return 1;
    }
    return 0;
}
...
/** 
 * Whether this BBL should be ignored.  By default, no BBL is ignored.  WHen the tool is run 
 * only to identify divergent instructions (where AMD and Intel can return different bitwise 
 * results), then most basic blocks can be ignored.  This routine will find the ones we want
 * to track
 */
static BOOL IgnoreBBL(BBL bbl)
{
    if( KnobOnlyDivergentAsm.Value() )
    {
        for (INS ins = BBL_InsHead(bbl); INS_Valid(ins); ins = INS_Next(ins))
        {
            if( IsDivergentOp( INS_Opcode(ins) ) ) {
                // Is a divergent op, so no, don't ignore this BBL
                return 0;
            }
        }
        // No divergent ops found, so yes, ignore this BBL
        return 1;
    } 
    else {
        // By default, don't ignore BBLs
        return 0;
    }
}
...
VOID Trace(TRACE trace, VOID* v)
{
    ...
    {
        // Check if we should ignore this BBL or not
        if( IgnoreBBL(bbl) ) continue;
...
static VOID DumpStats(ofstream& out, STATS& stats, BOOL predicated_true, BOOL print_zeros, const string& title)
{
    ...
    for (UINT32 i = 0; i < MAX_INDEX; i++)
    {
        ...
        if( IsDivergentOp(i) ) out << "       " << setw(16) << "<========   Divergent Ins  ==========";
        out << endl;
    }
}
